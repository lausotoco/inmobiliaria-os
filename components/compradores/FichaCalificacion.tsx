...
/* ============================================================
   Ficha de calificación del comprador (components/compradores/FichaCalificacion.tsx)
   Se muestra dentro de la ficha única del comprador (/clientes/[id],
   pestaña Calificación) y sigue funcionando sola.
   ------------------------------------------------------------
...
export default function FichaCalificacion({
  clienteId,
  embebida = false,
}: {
  clienteId: string;
  ...
  return (
    <div>
      {!embebida && (
        <Link href="/clientes" className="text-sm text-neutro transition hover:text-tinta">
          ← Compradores
        </Link>
      )}
      ...
      <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          {embebida ? (
            <p className="text-xs font-semibold uppercase tracking-widest text-laton">Calificación del comprador</p>
          ) : (
